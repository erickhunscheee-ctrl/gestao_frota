...
function mascaraCEP(el) {
  ...
  if (cepLimpo.length === 8) {
    buscarCidadePorCEP(cepLimpo);
  }
}

async function buscarCidadePorCEP(cep) {
  const cidadeEl = document.getElementById('op-cidade');
  if (!cidadeEl) return;
  
  cidadeEl.placeholder = "Buscando cidade...";
  try {
    ...
    if (data && !data.erro) {
      ...
      cidadeEl.value = "";
      cidadeEl.placeholder = "CEP não encontrado";
    }
  } catch (e) {
    console.error("Erro ao buscar CEP", e);
    cidadeEl.placeholder = "Erro ao buscar CEP";
  }
}
...
/* ?? MODAL FORMS (shared entre p?ginas) ?? */
const MODAL_FORMS = {
  operador: () => ({
    title: 'Novo Operador',
    sub: 'Cadastrar novo operador na plataforma',
    html: `<div class="form-grid">
      <div class="form-field"><label class="form-label">Nome completo *</label><input class="form-input" id="op-nome" placeholder="Ex: João da Silva"/></div>
      <div class="form-field"><label class="form-label">Cargo / Nível</label><select class="form-select" id="op-cargo"><option value="Operador Sênior">Operador Sênior</option><option value="Operador Pleno">Operador Pleno</option><option value="Operador Júnior">Operador Júnior</option></select></div>
      <div class="form-field"><label class="form-label">E-mail *</label><input class="form-input" id="op-email" type="email" placeholder="joao@empresa.com"/></div>
      <div class="form-field"><label class="form-label">Telefone</label><input class="form-input" id="op-telefone" placeholder="(51) 9 9999-9999" maxlength="16" oninput="mascaraTelefone(this)"/></div>
      <div class="form-field">
        <label class="form-label">CPF</label>
        <input class="form-input" id="op-cpf" placeholder="000.000.000-00" maxlength="14" oninput="mascaraCPF(this)"/>
        <span id="op-cpf-erro" style="display:none;font-size:11px;color:#EF4444;margin-top:4px;">CPF inválido</span>
      </div>
      <div class="form-field"><label class="form-label">Senha de acesso *</label><input class="form-input" id="op-senha" type="password" placeholder="Mínimo 6 caracteres"/></div>
      <div class="form-field"><label class="form-label">Obra / Unidade</label><input class="form-input" id="op-unidade" placeholder="Ex: Obra Norte"/></div>
      <div class="form-field"><label class="form-label">CEP</label><input class="form-input" id="op-cep" placeholder="00000-000" maxlength="9" oninput="mascaraCEP(this)"/></div>
      <div class="form-field"><label class="form-label">Cidade</label><input class="form-input" id="op-cidade" placeholder="Preenchida automaticamente pelo CEP" readonly/></div>
    </div>`
  }),

  maquina: () => ({
    title: 'Cadastrar Máquina',
    sub: 'Adicionar nova máquina',
    html: `<div class="form-grid">
      ...
      <div class="form-field"><label class="form-label">Ano de fabrição</label><input class="form-input" type="number" placeholder="2020"/></div>
      ...
      <div class="form-field"><label class="form-label">Status Inicial</label><select class="form-select"><option value="1">Ativa / Em uso</option><option value="2">Manutenção</option><option value="3">Inativa</option></select></div>
  ...
  obra: () => ({
    ...
    html: `<div class="form-grid">
      ...
      <div class="form-field"><label class="form-label">Horíametro total (R$)</label><input class="form-input" type="number" placeholder="350000"/></div>
  ...
  vincular: (obraId, obraNome) => ({
    ...
    html: `
    <p style="font-size:12px;color:var(--text-muted);margin-bottom:14px;">Selecione os operadores e m?quinas que ser?o alocados nesta obra.</p>
    <div style="margin-bottom:18px;">
      ...
      <div class="check-list">
        <label class="check-item"><input type="checkbox" checked/><span class="check-item-name">Jo?o Silva</span><span class="check-item-meta">S?nior ? Escavadeira 320D</span></label>
        ...
        <label class="check-item"><input type="checkbox"/><span class="check-item-name">Carlos Souza</span><span class="check-item-meta">J?nior ? Sem m?quina</span></label>
      </div>
    </div>
    <div>
      <div style="font-size:12px;font-weight:700;color:var(--text-primary);text-transform:uppercase;letter-spacing:0.4px;margin-bottom:8px;">M?quinas</div>
      <div class="check-list">
        ...
        <label class="check-item"><input type="checkbox"/><span class="check-item-name">Compactador CV213</span><span class="check-item-meta">CP-213 ? Dynapac ? 2018 ? ?? Em manuten??o</span></label>
